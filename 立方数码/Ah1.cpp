...
// 在结点tmp的后面插入node，如果node的f值比当前的f小，则将f值替换为node->f
...
void print_path(Node* end) {
	...
	stack<Node*> Stack;
	while (p) {	
		// 从最终状态结点开始，一个个地将状态结点压入栈中，直到初始状态结点
		Stack.push(p);
		p = p->parent;
	}
	while (!Stack.empty()) {
...
/* 错位的棋子数作为启发函数h1(n)的值 */
int h1(Node* start, Node* target) {
	int count = 0;
	for (int i = 0; i < 27; i++) {
		if (start->elem[i] != target->elem[i])
			count++;
	}
	return count;
}
...
/* 移动空格棋子的函数 */
void move_blank(Node* current_status, movement action, Node* target, movement last_move, Node* VISIT) {
	// 将下一次要访问的状态结点取出
...
void A_star(Node* start, Node* target) {
	...
	start->h = h1(start, target);	// 启发函数由专门的函数计算
	if (start->h == 0) {
		// 初始状态与目标状态相同，直接返回
	...
	start->parent = NULL;	// 初始状态没有父结点
	start->action = NONE;	// 进入初始状态不需要动作

	// 定义一些变量并初始化
	movement last_move = NONE;	// 上一次移动操作：无
	visited = 0;	// 当前访问次数为0
	Node* VISIT = new Node[MAXVISIT];	// 访问过的状态的数组

	// 将初始状态放入OPEN队列
	insert(start);
	long int step = 0;	// 探索的步数
	start_time = clock();	// 开始计时

	// 开启队列非空且搜索步数没有达到上限时进行搜索
	while (top() && step < MAXSTEP) {
		// 将当前开启队列的顶部元素弹出，放到关闭队列中去
		Node *p = top();
		// OPEN.top();
		// OPEN.pop();
		pop();
		CLOSE.insert(pair<int*, int>(p->elem, p->f));
		...
		for (int i = 0; i < 6; i++) {
			...
			move_blank(p, (movement)i, target, last_move, VISIT);
		}
	}
	// 遍历了所有可达状态而不能到达目标状态
